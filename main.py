    ...
    *,
    ...
    while True:
        ...
        for raw_call in function_calls:
            call = getattr(raw_call, "function_call", raw_call)
            name = getattr(call, "name", "<unknown_tool>")
            raw_args = getattr(call, "args", None)

            try:
                args = args_to_dict(raw_args)
            except ValueError as exc:
                invalid_request = format_tool_request(name, {"_raw_args": str(raw_args)})
                emit_message(client, role="model", text=invalid_request)
                invalid_result = f"[tool-response] {name} result: invalid tool args payload: {exc}"
                emit_message(client, role="user", text=invalid_result)
                continue
...
def main() -> None:
    ...
    api_key = load_api_key(prompt=True)
